...
stopRecordButton.addEventListener('click', async () => {
    ...
    statusArea.textContent = "Recording stopped. Press 'Send' to transcribe and send.";

    // 自动上传音频
    mediaRecorder.onstop = async () => {
        const transcription = await uploadAudio();
        if (transcription) {
            textArea.value = transcription; // 显示转录结果
            msgHistory.innerHTML += `<span style='opacity:0.5'><u>User(Voice):</u> ${transcription}</span><br>`;
        } else {
            textArea.value = "Transcription failed. Please try again.";
        }
